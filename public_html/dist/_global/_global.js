...
$(document).on('click','a.scroll-to-top',function(e){e.preventDefault();$("#content-wrapper").animate({scrollTop:0},1000);});$(document).on("click",".clear-cache",function(e){e.preventDefault();$.ajax("".concat(root,"/admin/ajax/clearCache"));});$('[data-bs-toggle="popover"]').each(function(i,el){new bootstrap__WEBPACK_IMPORTED_MODULE_1__["Popover"](el);});});window._t=function(key,args){if(args){return translations[key].format(args);}return translations[key];};String.prototype.format=function(){var a=this,b;for(b in arguments){a=a.replace(/%[a-z]/,arguments[b]);}return a;// Make chainable
};
